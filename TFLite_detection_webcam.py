...
def text_to_speech(objects_list):
    # Generate a unique filename based on the current timestamp
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    output_filename = f"output_{timestamp}.mp3"

    # Combine all detected objects into a single text
    speech_text = ", ".join(objects_list)

    # Convert text to speech
    tts = gTTS(text=speech_text, lang="en")
    tts.save(output_filename)

    # Play the saved audio file with a delay to avoid quick overwrites
...
def deletemp3_files():
    ...
    directory_path = r"C:\Users\Ghassen\Desktop\Object Detection"
...
while True:
    ...
    # Convert the list of detected objects to speech
    text_to_speech(objects_list)
